...
public class EmailBuilder {
	...
	public static String getUpdateUserMessage(HashMap<String, Object> map) {
		StringBuilder msg = new StringBuilder();

		msg.append("<HTML><BODY>");
		msg.append("<H4>User Updating is successful for ORS Project SUNRAYS Technologies</H4>");
		msg.append("<H1>Hi! Greetings from SUNRAYS Technologies!</H1>");
		msg.append(
				"<P>Congratulations for registering on ORS! You can now access your ORS account online - anywhere, anytime and enjoy the flexibility to check the Marksheet Details.</P>");
		msg.append(
				"<P>Log in today at <a href='http://ors.sunraystechnologies.com'>http://ors.sunraystechnologies.com</a> with your following credentials:</P>");
		msg.append("<P><B> First Name : " + map.get("fName") + "<BR>" + " Last Name : " + map.get("lName") + "<BR>"
				+ " " + "Login Id : " + map.get("login") + "<BR>" + " " + " Mobile Number :" + map.get("mobileNo")
				+ "<BR>" + " Gender : " + map.get("gender") + "<BR>" + "  "
						+ "DOB : " + map.get("DOB") + "<BR>" + " </B></p>");
